 ...
 * @param {String} [argDataType] - 데이터 유형 : 0(XML 타입), 1(Binary 타입),  2(SSV 타입), 3(JSON 타입)
 * @param {Boolean} [isCompress] - 압축통신 여부  (default:false)
 * @return N/A
 * @example
 ...
 * this.gfnTransaction("save", strSvcUrl, inData, outData, strArg, "fnCallback", true, 0, false);
 */ 
pForm.gfnTransaction = function(strSvcId, strSvcUrl, inData, outData, strArg, callBackFnc, isAsync, argDataType, isCompress)
{
	...
	var nDataType = 3;
	if(argDataType != null) {
		nDataType = argDataType;
	}
	var bCompress = false;
	if(isCompress != null) {
		bCompress = isCompress;
	}
	this.transaction( strMergeSvcID                 	//1.strMergeSvcID
					...
					, isAsync                     				//7.bAsync
					, nDataType                     			//8.nDataType : 0(XML 타입), 1(Binary 타입),  2(SSV 타입), 3(JSON 타입) --> HTML5에서는 Binary 타입은 지원안함
