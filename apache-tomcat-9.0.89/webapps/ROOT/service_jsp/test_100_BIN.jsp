...
<%
     ...
     *********************************************************/
    HttpPlatformRequest platformRequest = new HttpPlatformRequest(request.getInputStream(), PlatformType.CONTENT_TYPE_BINARY);
   ...
   // String strUserID   = inVariableList.getString("test");
    ...
    HttpPlatformResponse platformResponse = new HttpPlatformResponse(response, PlatformType.CONTENT_TYPE_BINARYCONTENT_TYPE_BINARY);   
...
//	}
   ...
   //Thread.sleep(100);
	...
	//String cookieName = cookies[3].getName();
	//String cookieValue = cookies[3].getValue(); 
	...
	/*   for(int i = 0; i < 1; i++)
	   {
			...
			outDataSet0.addColumn("useclientlayout_BIN",   DataTypes.STRING, 255); 
			outDataSet0.newRow();
       ...
       // Output Dataset 컬럼 정의
		outDataSet00.addConstantColumn("ConstColumn0", DataTypes.STRING, 30, "BIN");
		...
		outDataSet00.addColumn("useclientlayout_BIN",   DataTypes.STRING, 255); 
	   ...
	   for(int i = 0; i < 10; i++)
	   {
			...
			{
				outDataSet00.set(newRow, "Column0", "BINARY");
				outDataSet00.set(newRow, "Column1", "test");
			}
			outDataSet00.set(newRow, "Column2", "^"+i+"^");
			outDataSet00.set(newRow, "useclientlayout_BIN", "X"+i+"X");
	...
	// 쿠키 지정 name, value
	Cookie cookie = new Cookie("Cookie001", "BINARY서버쿠키테스트!!!");
	// 쿠키의 유효시간 설정
	cookie.setMaxAge(10);
	//생성된 쿠기에 path설정
