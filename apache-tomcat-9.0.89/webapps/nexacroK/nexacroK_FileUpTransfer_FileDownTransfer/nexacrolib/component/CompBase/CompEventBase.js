...
if (nexacro.Component)
{
	...
	_pComponent.on_click_default_action = function (navtarget, navurl, navtargetproperties)
	{
		...
		if (this._env && !this._env._p_preventbrowserbackbutton)	
		{
				...
				if (event_bubbles !== true)  
				{
					...
					if (!ctrl_key)
					{
						if (this._is_scrollable)			
						{
							...
							{
								if (this.on_mousewheel_default_action(wheelDeltaX, wheelDeltaY, shift_key)) //scroll 가능영역이 있는지 여부 체크 
								{
									let comp = this._getFromComponent(this);
									if (comp)
								...
								else
								{
									...
									if (wheelDeltaX)
										this._scrollTo(this._hscroll_pos, this._vscroll_pos, undefined, undefined, "", "mousewheel_h");
									//last 나 first 인 경우 상위로 scroll 을 넘겨야 하는 경우
								}
								//스크롤할 것이 없다면 상위로 onmousewheel 발생 필요
							}
							else
							{
								if (wheelDeltaY)
								...
								if (wheelDeltaX && nexacro.OS == "Mac OS")
								{
									this._is_wheel_scroll = true;
