...
if (!nexacro.FileDialog)
{
    ...
    {
        var vfiles = [];
        var _virtualFiles = objData.virtualfiles;

        for (var i = 0; i < _virtualFiles.length; i++)
        {
            var vfile = new nexacro.VirtualFile("vfile" + (new Date().valueOf().toString()) + i, "");

            vfile._setHandle(_virtualFiles[i]);
            vfile._setPath(_virtualFiles[i].path);
            vfile._setFullPath(_virtualFiles[i].fullpath);
            vfile._setFileName(_virtualFiles[i].filename);
            vfile._setLastModified(_virtualFiles[i].lastmodified);
