...
const nexaAppFactory = ismodule ? (function (global)
{
    ...
    apploader.createApp = function (rawSrc, load_modules)
    {
        ...
        class ObservedContaner
        {
            ...
            ContainerItem = class
            {
                ...
                static validateDataAttributes(element) 
                {
                    ...
                    {
                        if (attr == ObservedContaner.fdl_attr /*|| attr == ObservedContaner.css_attr*/) //data-css spec-out
                        {
                ...
                constructor(parent, node, _form = undefined, _css = undefined)
                {
                    ...
                    try
                    {
                        ...
                        this.sysevt = new nx.HTMLSysEvent(node);
                        this.sysevt._initDocEventHandler();


                        let launchFunc = undefined;
                        let ls_mainframe = false;
                        if (_form)
                        {
                            //if (_css)
                            //    nx.loadStyle(_css)
                            launchFunc = nx._loadFDL.bind(nx)

                        }
                        else
                        {
                            launchFunc = nx._loadDynamicMainframe.bind(nx)
                            ls_mainframe = true;
                        }
                        ...
                        launchFunc("", _form ? _form : undefined).then((from) =>
                        {
                            ...
                            if (ls_mainframe && this.app)
                            {
                                // mainframe 의 경우 후행 처리이기 때문에 container mapping을 끝내놓고 component 생성
                ...
                async close()
                {
                    ...
                    DispatchEvent(this.node, ObservedContaner.event("unloadcontainer", this.node.id ? this.node.id : this.node.nodeName, {}, ""));
            ...
            {
                const createDebouncedHandler = (callback, t) => 
                {
        ...
        class nexacroApp
        {
                ...
                */
                ...
                for (const path of modulePaths) 
                {
                    ...
                    catch (error) 
                    {
                        ...
                        nexacro._trace(`Failed to load module at path: ${path}, error: ${error.message}`);
            ...
            {
                let NEXACRO_REGISTRY_SYMBOL = global.NEXACRO_REGISTRY_SYMBOL;
                if (!bclear)
            ...
            static async loadframework(nx)
            {
                ...
                {
                    const retmodule = {};
                    if (!nx.__APPLOADER_BASE_PATH__)
                        nx.__APPLOADER_BASE_PATH__ = baseuri;
                    retmodule.nexacro = nx;
            ...
            */
            ...
            #waitApplicationCallbackResolve;

            createManager = () =>
            {
                if (!this.#manager)
                {
                    const { nx, env } = this.#exports;
                    this.#manager = new ObservedContaner(env.csscollisionpreventkey, nx);
                }
            }
            createApplication = async () =>
            {
                ...
                let adlFunc = nx._loadADL.bind(nx)
                this.#application = await adlFunc(""); // callback 을 하나 붙여야 겠다.
                const waitForCallback = new Promise((resolve) =>
                {
                    this.#waitApplicationCallbackResolve = resolve; // 외부로 resolve 저장
                });
            ...
            constructor(exports)
            {
                ...
                nexacro.__MFEAPI.loadManager = this.createManager
                nexacro.__MFEAPI.initApplicationCallback = async function ()
                {
                    if (this.#waitApplicationCallbackResolve)
                    {
                        this.#waitApplicationCallbackResolve();
                        this.#waitApplicationCallbackResolve = null;
                    }
                }.bind(this);
                ...
                nexacro.__MFEAPI._getApplication = function ()
                {
            ...
            onstartApp = async (event) =>
            {
                const { nx, env } = this.#exports



                DispatchEvent(document, nexacroApp.event("init", document.id, { nx }, ""));
                const pThis = this;
                queueMicrotask(() =>
                {
                    nexacro._prepareManagerFrame(window, document, async function ()
                    {
                        const key = env.csscollisionpreventkey;
                        await pThis.createApplication();
            ...
            oncloseApp = async (event) =>
            {
                ...
                if (bclose) 
                {
                    ...
                    for (let app of apps)
                    {
                        let win = app._getWindow();
                        win._closeChildWindows(true);
                    }
                    if (nexacro._popupframes)
                    {

                        const popupframes = nexacro._popupframes;
                        if (popupframes)
                        {
                            let child_len = popupframes.length;
                            for (let i = child_len - 1; i >= 0; i--)
                            {
                                let child = popupframes[i]._window;

                                if (child && child.root && child.root)
                                {
                                    child.root._destroy();
                                }
                            }
...
if (!nexacro._Init_systembase)
{
    ...
    else
    {
        ...
        */
        ...
        const parseVersion = (regex, fallback = -1) => {
            const m = ua.match(regex);
            return m ? (parseInt(m[1], 10) | 0) : fallback;
        };
        ...
        if (/Edg\//.test(ua)) 
        {
            nexacro._Browser = "Edge";
            nexacro._BrowserType = "WebKit";
            nexacro._BrowserVersion = parseVersion(/Edg\/([.\d]+)/);
        }
        else if (/Chrome\//.test(ua)) 
        {
            nexacro._Browser = "Chrome";
            nexacro._BrowserType = "WebKit";
            nexacro._BrowserVersion = parseVersion(/Chrome\/([.\d]+)/);
        
            if (/Samsung/.test(ua)) nexacro._BrowserExtra = "SamsungBrowser";
            else if (/NAVER/.test(ua)) nexacro._BrowserExtra = "NaverBrowser";
            if (/XiaoMi/.test(ua)) nexacro._BrowserExtra = "MiuiBrowser";
        }
        else if (/CriOS\//.test(ua)) 
        {
            nexacro._Browser = "Chrome";
            nexacro._BrowserType = "WebKit";
            nexacro._BrowserVersion = parseVersion(/CriOS\/([.\d]+)/);
        
            if (/Samsung/.test(ua)) nexacro._BrowserExtra = "SamsungBrowser";
        }
        else if (window.opera || /Opera|OPR/.test(ua)) 
        {
            nexacro._Browser = "Opera";
            nexacro._BrowserType = "Opera";
            nexacro._BrowserVersion = parseVersion(/Version\/([.\d]+)/);
        }
        else if (/Apple.*Mobile/.test(ua) || (navigator.platform === "MacIntel" && navigator.maxTouchPoints > 0)) 
        {
            nexacro._Browser = "MobileSafari";
            nexacro._BrowserType = "WebKit";
        
            const mobileapple = /CPU.*OS ([0-9_]{1,5})|(CPU like).*AppleWebKit.*Mobile/i.exec(ua);
            nexacro._BrowserVersion = parseFloat((mobileapple?.[1] ?? "3_2").replace(/_/g, ".")) || -1;
        }
            ...
            nexacro._Browser = "WebKit";
            nexacro._BrowserType = "WebKit";
            nexacro._BrowserVersion = parseVersion(/WebKit\/([.\d]+)/);
        }
        else if (/Gecko\//.test(ua)) {
            nexacro._Browser = "Gecko";
            nexacro._BrowserType = "Gecko";
            nexacro._BrowserVersion = parseVersion(/rv:(.+?)[);]/);
        }
    }
    //==============================================================================
    ...
    // Component의 Default로 변경 (IE 제거로 필요없음)
    //nexacro._use_translate_scroll = true;
    nexacro._use_legacy_icontext = (nexacro._Browser == "Runtime"); // incontext 구조 개선 이전 버전 사용 여부, 웹툴 개발 전까지 Runtime 호환성 유지 또는 신/구 버전 비교 목적, 웹툴 개발 후 제거 예정
    nexacro._use_track_overlay = true; // mouse track overlay 사용 여부
                ...
                get(target, prop, receiver) 
                {
                    if (prop === 'getApplication' || prop === 'getMainObject')
                    {
                        return () => 
                        {
                            ...
                            {
                                ret = nexacro.__getMainObject(_scope_context, true);
                                if (!ret)
                                    ret = nexacro.getApplication();
                            }
                            else
                                ret = nexacro.__getMainObject(_scope_context);
    ...
    nexacro.__getProcessExportArgs = function (context)
    {
        ...
        return Object.values({ nexacro: nexacro, system, trace });
    };
    ...
    _pEventSinkObject._getApplication = function ()
    {
    ...
    _pEventSinkObject._getRootObject = function ()
    {
        ...
        if (pThis)
        {
            return pThis._getRootObject(); // chk
        }
        return null;
    };
    ...
    //==============================================================================
    // nexacro.StyleAnimationEventInfo
    //==============================================================================
    nexacro.StyleAnimationEventInfo = function (obj, id, animationname, elapsedtime, from_comp, from_refer_comp)
    {
        this.id = this.eventid = id || "onanimationend";
    ...
    var _pStyleAnimationEventInfo = nexacro._createPrototype(nexacro.Event, nexacro.StyleAnimationEventInfo);
    nexacro.StyleAnimationEventInfo.prototype = _pStyleAnimationEventInfo;
    _pStyleAnimationEventInfo._type_name = "StyleAnimationEventInfo";
    ...
    {
        this.id = this.eventid = id || "onorientationchange";
        this.fromobject = null;
        this.fromreferenceobject = null;
    ...
    // RootCssClassChangeEventInfo
    nexacro.RootCssClassChangeEventInfo = function (obj, id, precssclass, postcssclass)
    {
        this.id = this.eventid = id || "onrootcssclasschange";
        this.fromobject = null;
        this.fromreferenceobject = null;

        this.precssclass = precssclass;
        this.postcssclass = postcssclass;
    };
    var _pRootCssClassChangeEventInfo = nexacro._createPrototype(nexacro.Event, nexacro.RootCssClassChangeEventInfo);
    nexacro.RootCssClassChangeEventInfo.prototype = _pRootCssClassChangeEventInfo;
    _pRootCssClassChangeEventInfo._type_name = "RootCssClassChangeEventInfo";
    ...
    //nexacro.TouchSession._default_gesture_detectors = ['longpress',  'tap'];
    ...
    {
        return false;  // unsupport logical fling
    ...
    _pTouchActionSlide._on_action_fling = function (/*status*/)
    {
        ...
        if (scroll_comp._is_scrollable && scroll_comp._use_native_scroll)
            return;
...
//			return window.btoa(input);
    ...
    nexacro._getCSSPredefineValue = function (id, type)
    {
        let value = undefined;
        ...
        const rootcssclasslist = nexacro._getRootCSSClass();
        ...
        nexacro._setCSSPredefineSpecialConvInfo(id, type, value);
    ...
    nexacro._setCSSPredefineMaps = function (map)
    {
        ...
        nexacro._refreshAllRootCSSVariables();
    };
    ...
    nexacro._setCSSPredefineGroupAutoSelectInfo = function (obj)
    {
        if (!obj)
            return;
        ...
        if (!env._css_group_autoselect_info)
            env._css_group_autoselect_info = obj;
        else
            Object.assign(env._css_group_autoselect_info, obj);
    };

    nexacro._setRootCSSVariable = function (nodeStyle, varname, value)
    {
        nodeStyle.setProperty(`--${varname}`, value);
    };

    nexacro._setCSSPredefineSpecialConvInfo = function (id, type, value)
    {
        if (value === undefined)
            return;
        ...
        if (!nexacro._css_predefine_special_convert_map)
            nexacro._css_predefine_special_convert_map = new Map();
        ...
        if (!convertInfo)
        {
            ...
            if (!rafScheduled)
            {
                ...
                {
                    document.querySelectorAll(`.${env.themekey}.${env.csscollisionpreventkey}`)
                        .forEach(rootNode =>
                        {
    ...
    nexacro._updateRootCSSVariable = function (nodeStyle, id, type, value)
    {
        const _setCSSVariableHandler = {
            wordwrap: (style, nodeid, nodevalue) => 
            {
                // wordwrap : white-space, word-break, word-wrap
                if (nodevalue == "char")
                {
                    nexacro._setRootCSSVariable(style, "white-space-" + nodeid, "pre-wrap");
                    nexacro._setRootCSSVariable(style, "word-break-" + nodeid, "break-all");
                    nexacro._setRootCSSVariable(style, "word-wrap-" + nodeid, "break-word");
                }
                else if (nodevalue == "english")
                {
                    nexacro._setRootCSSVariable(style, "white-space-" + nodeid, "pre-wrap");
                    nexacro._setRootCSSVariable(style, "word-break-" + nodeid, "normal");
                    nexacro._setRootCSSVariable(style, "word-wrap-" + nodeid, "break-word");
                }
                else // none
                {
                    nexacro._setRootCSSVariable(style, "white-space-" + nodeid, "pre");
                    nexacro._setRootCSSVariable(style, "word-break-" + nodeid, "normal");
                    nexacro._setRootCSSVariable(style, "word-wrap-" + nodeid, "normal");
                }
            },
            edge: (style, nodeid, nodevalue) =>
            {
                // edge : border-image
                const arr = nodevalue.split(' ');
                if (arr.length > 1)
                {
                    const image = arr[0];
                    const fixedwidth = arr.length > 1 ? arr[1] : "0px";
                    const fixedheight = arr.length > 2 ? arr[2] : "0px";

                    nodevalue = `${image} ${fixedwidth.replace('px', '')} ${fixedheight.replace('px', '')} fill / ${fixedwidth} ${fixedheight}`;
                }
                else // none
                {
                    nodevalue = "none 100% / 0px";
                }
                nexacro._setRootCSSVariable(style, "border-image-" + nodeid, nodevalue);
            }
        };

        const _setCSSVariable = (style, nodeid, nodetype, nodevalue) =>
        {
            const handler = _setCSSVariableHandler[nodetype];
            if (handler)
            {
                handler(style, nodeid, nodevalue);
            }
        };

        _setCSSVariable(nodeStyle, id, type, value);
    };

    nexacro._refreshAllRootCSSVariables = function ()
    {
        const env = nexacro.getEnvironment();
        if (!env)
            return;
        ...
        document.querySelectorAll(`.${env.themekey}.${env.csscollisionpreventkey}`).forEach(rootNode =>
        {
    ...
    _pDeviceAdaptor.fireEvent = function (eventid, eventtype, deviceid, data)	
    {
        ...
        {
            let target = this.parent._getRootObject(); // chk
    ...
    nexacro._setTrackInfo = function (targetwin, target, windowX, windowY, screenX, screenY, no_useoverlay)
    {
        ...
        if (!no_useoverlay)
            nexacro._setTrackOverlay(true);
    };

    nexacro._initTrackInfo = function ()
    {
        nexacro._cur_track_info = null;

        nexacro._setTrackOverlay(false);
    }

    nexacro._setExtraTrackInfo = function (targetwin, target, windowX, windowY, screenX, screenY, keepstart, workkind)
    {
        var trackData = target._on_start_extratrack(windowX, windowY, screenX, screenY, keepstart, workkind);
    ...
    nexacro._getDeviceName = function ()
    {
        ...
        while (i < devices.length && !matches)
        {
            ...
            {
                matches = regex[j++].exec(useragent);
                if (matches)
                {
    ...
    nexacro._applySelectedScreen = function (curscreen)
    {
        ...
        //PinchZoom
        let allow_default_pinchzoom = curscreen.userzoom == "true" ? true : false;
    ...
    */
    ...
    nexacro._getParsedUri = function (uri)
    {
        ...
        const re = new RegExp("^((\\w+):)?(//((\\w+)?(:(\\w+))?@)?([^/?:]+)(:(\\d+))?)?(/?([^/?#][^?#]*)?)?(\\?([^#]+))?");
        var ret = uri.match(re);
    ...
    nexacro._Deserializer = {
        "SSV": function (strRecvData, target)
        {
            ...
            function __find_next_dataset_loopFn(_index)
            {
                var _str = ssvLines[_index];
                if (_str.substring(0, 7) == "Dataset")
                {
                    return true;
                }
            }
    ...
    nexacro._JSONPath = function (obj, expr, arg)
    {
        ...
        var P = {
            ...
            result: [],
            normalize: function (exprval) 
            {
                var subx = [];
                return exprval.replace(/(?:\[|')(\??\(.*?\))(?:\]|')/g, (_m, group) => `[#${subx.push(group) - 1}]`)
                    .replace(/'?\.'?|\['?/g, ";")
                    .replace(/;;;|;;/g, ";..;")
                    .replace(/;$|'?\]|'$/g, "")
            ...
            track: function (exprval, val, path) 
            {
                if (exprval)
                {
                    var x = exprval.split(";"), loc = x.shift();
                    x = x.join(";");
                    if (val && Object.prototype.hasOwnProperty.call(val,loc))
                        P.track(x, val[loc], path + ";" + loc);
                    else if (loc === "*")
                        P.walk(loc, x, val, path, function (m, l, _x, v, p) { P.track(m + ";" + _x, v, p); });
                    else if (loc === "..")
                    {
                        P.track(x, val, path);
                        P.walk(loc, x, val, path, function (m, l, _x, v, p) { typeof v[m] === "object" && P.track("..;" + _x, v[m], p + ";" + m); });
                    }
                    else if (/,/.test(loc)) 
                    { // [name1,name2,...]
                    ...
                    else if (/^\(.*?\)$/.test(loc)) // [(exprval)]
                        P.track(P.newfunc(loc, val, path.substr(path.lastIndexOf(";") + 1)) + ";" + x, val, path);
                    else if (/^\?\(.*?\)$/.test(loc)) // [?(exprval)]
                        P.walk(loc, x, val, path, function (m, l, _x, v, p) { if (P.newfunc(l.replace(/^\?\((.*?)\)$/, "$1"), v[m], m)) P.track(m + ";" + _x, v, p); });
            ...
            walk: function (loc, exprval, val, path, f) 
            {
                if (val instanceof Array) 
                {
                    for (var i = 0, n = val.length; i < n; i++)
                        if (i in val)
                            f(i, loc, exprval, val, path);
                }
                else if (typeof val === "object") 
                {
                    for (var m in val)
                        if (Object.prototype.hasOwnProperty.call(val,m))
                            f(m, loc, exprval, val, path);
                }
            },
            slice: function (loc, exprval, val, path) 
            {
                if (val instanceof Array) 
                {
...
else if (nexacro._macOSWebView)
{
    nexacro._messageToNative = function (id, div, method, params, is_sync, is_get)
    {
        ...
        if (is_get === true)
            return nexacro.Device.exec(jsonstr, is_sync);
        else
            nexacro.Device.exec(jsonstr, is_sync);
    };
}
...
if (_process)
{
    ...
    _pRootCssClassChangeEventInfo = null;
