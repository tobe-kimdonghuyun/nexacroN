(function()
{
    return function()
    {
        ...
        this.on_create = function()
        {
            ...
            obj.set_useclientlayout("true");
            obj._setContents("<ColumnInfo><Column id=\"cmdtySn\" type=\"STRING\" size=\"256\"/><Column id=\"fileMngNo\" type=\"STRING\" size=\"256\"/><Column id=\"fileFullName\" type=\"STRING\" size=\"256\"/><Column id=\"orgnlFileNm\" type=\"STRING\" size=\"256\"/><Column id=\"fileFlpth\" type=\"STRING\" size=\"256\"/><Column id=\"fileSz\" type=\"STRING\" size=\"256\"/><Column id=\"colId\" type=\"STRING\" size=\"256\"/></ColumnInfo>");
            this.addChild(obj.name, obj);
            ...
            obj = new Layout("default","",0,0,this.divForm.form.divMain.form,
                ...
                p.Grid00.set_scrolltype("both");
                p.Grid00.set_scrollpixel("default");
        ...
        // User Script
        this.addIncludeScript("CST012M00.xfdl","xjs::TaskCom.xjs");
        this.registerScript("CST012M00.xfdl", function() {
        /**
        ...
        *******************************************************************************************************************************/
        this.fnSearch = function () {
        	...
        	var outData     = "dsList=dsList dsAtfl=dsAtfl";
        	var strArg      = "";
        ...
        this.fnSave = function () {
        	...
        	var inData      = "dsList=dsList:A dsAtfl=dsAtfl:U";
        ...
        *******************************************************************************************************************************/
        this.fnCallback = function(svcId, errCd, errMsg)
        {
        	...
        	switch(svcId) {
        		case "search" :
        			this.divForm.form.staTotal01.text = this.dsList.getRowCount();
        ...
        *******************************************************************************************************************************/
        ...
        // 첨부파일 셋팅
        this.fnMakeRaonKInfo = function (nRow) {

        	if(this.dsList.rowcount == 0 || this.dsAtfl.rowcount == 0) {
        		return;
        	}
        	//trace("fnMakeRaonKInfo = " + nRow);

        	this.dsFile.clearData();
        	//파일 표출용
        	this.dsFile.addRow();
        	this.dsFile.setColumn(0, "cmdtySn", this.dsList.getColumn(nRow, "cmdtySn"));
        	this.dsFile.setColumn(0, "colId", this.dsList.getColumn(nRow, "cmdtySn"));

        	var atflRow;
        	if(this.dsList.getRowType(nRow) == Dataset.ROWTYPE_INSERT) {
        		atflRow = this.dsAtfl.findRow("colId", this.dsList.getColumn(nRow, "maxSeq"));
        	} else {
        		atflRow = this.dsAtfl.findRow("colId", this.dsList.getColumn(nRow, "cmdtySn"));
        	}

        	if(atflRow == -1) { return; }
        	this.dsFile.setColumn(0, "orgnlFileNm", this.dsAtfl.getColumn(atflRow, "orgnlFileNm") + '(' + this.dsAtfl.getColumn(atflRow, "fileSz") + ')');  // 파일풀명

        	//첨부파일 이미지 보여주기 위해서는 여기서 진행 필요!
        ...
        *******************************************************************************************************************************/
        ...
        {
        	if (!this.gfnDsIsUpdated(this.dsList) && !this.gfnDsIsUpdated(this.dsAtfl)) {
        ...
        {
        	const oTargetDs = this.divForm.form.grdList.getBindDataset();
        	...
        	nRows.reverse().forEach(nRow =>{
        		var atflColId;
        		if(oTargetDs.getRowType(nRow) == Dataset.ROWTYPE_INSERT) {
        			atflColId = oTargetDs.getColumn(nRow, "maxSeq");
        		} else {
        			atflColId = oTargetDs.getColumn(nRow, "cmdtySn");
        		}
        		this.RaonkUpload.upatflinfo = {colid : atflColId};
        ...
        {
        	//파일관련 정보
        	if (this.dsList.getColumnInfo("maxSeq") == null) {
        		this.dsList.addColumn("maxSeq");
        	}
        	var maxSeq = this.gfnNvl(this.dsList.getMax("nexacro.toNumber(maxSeq, 0)"), 0) + 1;
        	var nRow = this.dsList.rowposition;
        	var colId;

        	//사용자물품일련번호가 생성된 건이라면 해당 정보로 세팅
        	if(!this.gfnIsNull(this.dsList.getColumn(nRow, "cmdtySn"))) {
        		colId = this.dsList.getColumn(nRow, "cmdtySn");
        	} else {
        		//아직 생성되기 전이라면 maxSeq로 세팅
        		colId = maxSeq;
        		this.dsList.setColumn(nRow, "maxSeq", maxSeq);
        	}

        	// ! key 값으로 cmdtySn 또는 maxSeq 사용
        	this.RaonkUpload.upatflinfo = {colid : colId};
        	this.RaonkUpload.OpenFileDialog();	//upatflinfo를 별도로 설정한 경우는 cfnSetAtflInfo를 사용하지 않음
        };

        //파일 업로드 관련(2024-12-20)
        this.RaonkUpload_RAONKUPLOAD_CreationComplete = function(obj,  paramObj)
        {
        	...
        	//이하 아래의 내용들 함수 안에서 바로 this로 접급 못해서 밖에서 우선 선언
            ...
            obj.cfnAddAtfl = function(oFile) {
        		...
        		dsAtfl.setColumn(nRow, "orgnlFileNm", oFile.userdata.strName);
        		dsAtfl.setColumn(nRow, "fileSz", oFile.userdata.nSize);

        		// 첨부 파일 표출용 DS 설정
        		dsFile.clearData();
        	    dsFile.addRow();
        		var nRowposition = dsList.rowposition;
        		dsFile.setColumn(0, "cmdtySn", dsList.getColumn(nRowposition, "cmdtySn"));
        		dsFile.setColumn(0, "colId", this.upatflinfo.colid);
        		dsFile.setColumn(0, "orgnlFileNm", oFile.userdata.strName + '(' + oFile.userdata.nSize + ')');  // 파일풀명
        	};
        ...
        this.Grid_oncellclick = function(obj,e)
        {
        	...
        	// 그리드 내의 버튼 클릭
        	if(e.col == 0) {
        		if(this.dsList.getRowType(nRow) == 1 && obj.getCellPropertyValue(0, 0, "cssclass") == "CellFile") {  // 파일
        			// 파일 다운로드
        			this.cfnFileDwnld(this.RaonkUpload, this.dsAtfl, this.dsList.getColumn(nRow, "cmdtySn"), e);
        		}
            } else if(e.col == 1) {
        		if(obj.getCellPropertyValue(0, 1, "edittype") != "none") {
        			if(obj.getCellPropertyValue(0, 1, "cssclass") == "CellFileUp") {  // 파일찾기
        				...
        				if(!this.gfnIsNull(this.dsList.getColumn(nRow, "cmdtySn"))) {
        					...
        					//아직 생성되기 전이라면 maxSeq로 세팅
        					colId = this.dsList.getColumn(nRow, "maxSeq");
        				}
        ...
        this.dsList_onrowposchanged = function(obj,e)
        {
        	...
        	//trace("obj.rowposition = " + obj.rowposition);
        	this.fnMakeRaonKInfo(obj.rowposition);
        };
        ...
        this.dsAtfl_onload = function(obj,e)
        {
        	...
        	if(this.RaonkUpload) {
        		if(this.dsAtfl.rowcount > 0) {
        			...
        			//그리드의 orgnlFileNm 생성
        			this.dsList.addColumn("orgnlFileNm", "string");
        		}
        	}
        	...
        	for(var i = 0; i < this.dsAtfl.rowcount; i++) {
        		...
        		var row = this.dsList.findRow("cmdtySn", colId);
        		this.dsList.setColumn(row, "orgnlFileNm", orgnlFileNm);
        ...
        this.loadIncludeScript("CST012M00.xfdl");
