...
(function()
{
    return function(path)
    {
        ...
        * @param objDs : 데이타셋
        *        objGrid : 그리드
        *        allChkYn : Y.전체라인체크 N.변경라인만체크
        * @return true/false
        * @example if (this.cfnGrdIsValidated(this.divGrd.form.grdList) == false) return
        */
        this.cfnGrdIsValidated = function(objGrid, objDs, allChkYn) {
        	for (var i = 0; i < objGrid.rowcount; i++) {
        		if (objDs == null || allChkYn == "Y" || (objDs != null && (objDs.getRowType(i) == "2" || objDs.getRowType(i) == "4"))) {
        			for (var j = 0; j < objGrid.getCellCount("head"); j++) {
        				if (objGrid.getCellPropertyValue(-1,j,"cssclass" ) == "CellHead_E") {
        					...
        					} else if (objGrid.getCellPropertyValue(i,j,"edittype" ) == "date") {
