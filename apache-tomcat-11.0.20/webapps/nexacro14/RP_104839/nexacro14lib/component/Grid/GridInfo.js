...
if (!nexacro.GridFormat) {
	...
	_pGridCellInfo._addStyleCache = function (styleid, propobj) {
		if (!this._stylecache[styleid] && propobj && !propobj._is_empty) {
	...
	_pGridCellInfo._getTooltipType = function () {
		var type = this.tooltiptype;
		if (!type || type == "") {
			var bandinfo = this.parent;
			type = bandinfo._getTooltipType();
		}
		return type;
	};
			...
			if (this.__isMask(maskCh) || maskCh == "\\" || maskCh == "\"" || maskCh == "\'") {
				...
				if (n < valueLen) {
					while (n < valueLen) {
						...
						if (maskDelimiter && maskDelimiter == ch) {
							i--;
							break;
						}
	...
	_pGridCellInfo._getDisplayText_number = function (rowidx) {
		...
		if (!isNaN(v)) {
			v = v.toString();
			v = v.replace(masknumber_api._default_decimal_point, masknumber_api._strPoint);
	...
	_pGridBandInfo.tooltiptype = "default";
	...
	_pGridBandInfo._find_gridpseudo_obj = function (styleProp, pseudo, celltype, stylePropParent, bNotFindGrid, returnType) {
		if (this.bandctrl) {
			...
			if (!bNotFindGrid && (obj == null || (obj && obj._bindtype > 0))) {
	...
	_pGridBandInfo._getTooltipType = function () {
		var type = this.tooltiptype;
		if (!type || type == "") {
			type = this.grid.tooltiptype;
		}
		return type;
	};
		...
		if (this._cols) {
			...
			if (!autofitcol_rate.length) {
				...
				for (var i = 0; i < _colsLen; i++) {
					...
					if (this.grid._calc_decimal) {
						_cols[i].size = autofitcol_rate[i] * this._orgbodyWidth;
					}
					else {
	...
	_pGridFormat._insertHeadRow = function (addidx, size) {
		...
		this._adjustFormatRowSize(this._headrows, true);
		return row;
	};
	...
	_pGridFormat._insertSummRow = function (addidx, size) {
		...
		this._adjustFormatRowSize(this._summrows, true);
		return row;
	};
	...
	_pGridFormat._insertBodyRow = function (addidx, size) {
		...
		this._adjustFormatRowSize(this._bodyrows, true);
		return row;
	};
	...
	_pGridFormat._adjustColWidth = function (bodywidth, autofitcol_rate, is_body) {
		if (bodywidth <= 0) {
			return false;
		}
		...
		if (this._bodyWidth != bodywidth && is_body) {
			retn = true;
		...
		if (!autofitcol_rate.length) {
			for (var i = 0; i < len; i++) {
				...
				col.left = grid._calc_decimal ? Math.floor(pos) : pos;
				...
				col.size = col.size * factor;

				autofitcol_rate[i] = col.size / bodywidth;

				if (grid._calc_decimal) {
					pos = pos + col.size;


					col.right = Math.floor(pos);
					col.size = col.right - col.left;
					tot += col.size;
				}
				else {
					if (col.size >= 0.5) {
		...
		if (bodylastcol && bodylastcol.size > 0) {
			if (bodylastcol.right != bodywidth) {
				var gap = bodywidth - tot;
				bodylastcol.tempsize = bodylastcol.size;
				bodylastcol.size += gap;
				bodylastcol.right += gap;
			}
			...
			if (merged_matrix[i]) {
				...
				if (bKeepSubCell) {
					if (bandcells[i]._col == startCol && bandcells[i]._row == startRow) {
						...
						var bandstyle = bandcells[i].style;
						var style = cellinfo.style;
						if (bandstyle.line) {
							style.set_line(bandstyle.line._value);
						}
						if (bandstyle.linetype) {
							style.set_linetype(bandstyle.linetype._value);
						}
						if (bandstyle.background) {
							style.set_background(bandstyle.background._value);
						}
						if (bandstyle.background2) {
							style.set_background2(bandstyle.background2._value);
						}
						if (bandstyle.gradation) {
							style.set_gradation(bandstyle.gradation._value);
						}
						if (bandstyle.gradation2) {
							style.set_gradation2(bandstyle.gradation2._value);
						}
						style._updateValue();
						...
						if ((current_merge_col >= nStartCol && current_merge_col <= nEndCol) && (current_merge_row >= nStartRow && current_merge_row <= nEndRow)) {
							if ((band_cells[i]._rowspan * band_cells[i]._colspan) > 1) {
								...
								for (var j = 0; j < (band_cells[i]._rowspan * band_cells[i]._colspan); j++) {
									...
									matrix.splice(make_cell._cellidx, 0, make_cell);
								}
							}
							else {
								band_cells[i]._cellidx = col + (col_count * row);
								matrix.splice(band_cells[i]._cellidx, 0, band_cells[i]);
							}
						}
						else {
							matrix.push(band_cells[i]);
	...
	_pGridFormat._adjustFormatRowSize = function (rows, no_update_xml) {
		var len = rows.length;
		var bodyOffset = 0;
		var row = null;
		for (var i = 0; i < len; i++) {
